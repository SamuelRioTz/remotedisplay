...
// con CGConfigureDisplayWithDisplayMode + kCGConfigureForSession. ¿Persiste tras
...
static bool setEnabled(uint32_t id, bool on) { void *h = dlopen("/System/Library/PrivateFrameworks/SkyLight.framework/SkyLight", RTLD_LAZY); RDSetEnabledFn f = h ? (RDSetEnabledFn)dlsym(h, "CGSConfigureDisplayEnabled") : NULL; if (!f) { printf("  sin CGSConfigureDisplayEnabled\n"); return false; } CGDisplayConfigRef c; if (CGBeginDisplayConfiguration(&c) != kCGErrorSuccess) return false; int rc = f((CGDirectDisplayID)(uintptr_t)c, on); (void)rc; return CGCompleteDisplayConfiguration(c, kCGConfigureForSession) == kCGErrorSuccess; }
...
int main() { @autoreleasepool {
    ...
    printf("selectRetina(642x351, Session) -> %d\n", selectRetina(a, 642, 351, kCGConfigureForSession)); sleep(3); info("tras seleccionar retina", a);
    MacResizeVirtualDisplay(a, 800, 450); sleep(3); info("resize 800x450 (applySettings)", a);
    printf("selectRetina(800x450, Session) -> %d\n", selectRetina(a, 800, 450, kCGConfigureForSession)); sleep(3); info("tras seleccionar retina 2", a);
    active("antes de destruir"); MacDestroyVirtualDisplay(a); sleep(10); active("tras destruir A +10s (fantasma?)");
    uint32_t b = MacCreateVirtualDisplay(642, 351, 60, true, "RD retina B"); sleep(3); info("creado B", b);
    printf("selectRetina B -> %d\n", selectRetina(b, 642, 351, kCGConfigureForSession)); sleep(3); info("B tras seleccionar", b);
    printf("ocultar B (CGSConfigureDisplayEnabled) -> %d\n", setEnabled(b, false)); sleep(4); active("B oculto (deberia faltar)");
    printf("mostrar B -> %d\n", setEnabled(b, true)); sleep(4); info("B de vuelta", b); active("B visible");
    setEnabled(b, false); sleep(3); MacDestroyVirtualDisplay(b); sleep(8); active("B: oculto+destruido +8s");
    return 0;
} }
